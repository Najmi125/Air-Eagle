...
- Test harness built: pytest + conftest.py with an isolated,
  ...
  4/4 tests passing against real Postgres (not simulated).
...
- pcaa_ano012_core.py and duty_summary.py are slated to port in the
  NEXT session — not yet done as of this snapshot. Both were
  genuinely well-built in the old repo; they get tests written
  against them as they come in, not blind-ported.
...
## Current active task
Phase 1 (scaffolding) — this snapshot. Phase 2 (port core legality +
duty logic, with tests) is next, pending confirmation.

## Files changed
Everything — new repo. See directory structure below.
...
## Tests passed
4/4 — tests/test_migrations.py, against real Postgres 16
...
- Waiting on: Air Eagle's actual route network — blocks duty
  ...
  meantime, per phase plan)
- Open regulatory question, not yet confirmed with operator: does
  ANO-012 D21 (charter rest) apply to the scheduled-flight portion
  of ops too, or does scheduled flying fall under D20 (home/away
  base rest) instead? This determines real rule-engine behavior for
  the scheduled side and should be confirmed before Phase 3 schema
  work assumes an answer.
...
## Next safest step
Phase 2: port core/legality/pcaa_ano012_core.py and
core/duty_summary.py from the old repo, with tests written against
them as part of the port (not after). See phase plan in project
chat, 2026-07-19, for the full 7-phase sequence.
